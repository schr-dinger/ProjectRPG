...
{
    if (AggroTable.Contains(Player))
    {
        AggroTable[Player] += Amount;
    }
    else
    {
        AggroTable.Add(Player, Amount);
    }
...
{
    GEngine->AddOnScreenDebugMessage(-1, 1.0f, FColor::MakeRandomColor(), FString::Printf(TEXT("Perception")));
    ...
    Blackboard->SetValueAsObject("Target", PlayerList[0]);
