...
{
	Super::BeginPlay();
	GEngine->AddOnScreenDebugMessage(-1, 1.0f, FColor::MakeRandomColor(), FString::Printf(TEXT("Begin!")));

	//selectW = Cast<UCharacterSelectWidget>(CreateWidget(GetWorld(), selectWidgetClass));
	selectW = Cast<UCharacterSelectWidget>(CreateWidget(this, selectWidgetClass));

	if (selectW != nullptr)
	{
		GEngine->AddOnScreenDebugMessage(-1, 1.0f, FColor::MakeRandomColor(), FString::Printf(TEXT("Widget!")));
		selectW->AddToViewport();
		//selectW->AddToPlayerScreen();
	}
